...
<body>
    <%
        ...
        Reply reply = new ReplyDAO().getReply(replyID);
            ...
            if (!userID.equals(reply.getUserID())) { // !userID세션값과 bbs.getUserID 작성한사람 동일한지
                PrintWriter script = response.getWriter();
                script.println("<script>");
            ...
            // 매개변수로서 글제목 내용을 넘겨서 빈공간 확인
                if (request.getParameter("replyContent") == null || request.getParameter("replyContent").equals("")) {
                    ...
                    ReplyDAO RepDAO = new ReplyDAO(); //데이터 베이스에 접근 가능한 객체생성
                    int result = RepDAO.update(replyID, request.getParameter("replyContent"));
